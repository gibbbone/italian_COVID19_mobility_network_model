...
## By Francesco
BIGGEST_MSAS = ['Lombardia'] #['Lombardia','Lazio', 'Campania']

# BIGGEST_MSAS = ['Treviso','Padova','Roma','Firenze','Torino', 'Milano', 'Napoli', 'Bergamo','Varese']
#['Varese','Bergamo']
#['Milano', 'Roma', 'Napoli','Firenze']
#['Milano', 'Roma','Torino','Brescia','Napoli','Treviso','Firenze','Padova']

COMPUTER_DICT = {
    'bonaccorsi': ['CRISIS4'],
    'gibbone': ['DESKTOP-2U7PSI7'],
    "frapierri": ['MBPdiFrancesco2.lan', 'MBPdiFrancesco2.homenet.telecomitalia.it'],
    "fpierri" : ["vaccines-server"],
    "pierri" : ["geco"]
}
COMPUTER_STATS =  {
    'vaccines-server':16,
    ...
    'MBPdiFrancesco2.homenet.telecomitalia.it' : 2,
    'CRISIS4':60}
...
BETA_AND_PSI_PLAUSIBLE_RANGE = {
    "min_home_beta": 0.0011982272027079982,
...
UNZIPPED_DATA_DIR = os.path.join(BASE_DIR, 'all_aggregate_data/monthly_patterns_data/')
...
PATH_TO_OVERALL_HOME_PANEL_SUMMARY = os.path.join(UNZIPPED_DATA_DIR, 'summaries', 'visit_panel_summary.csv')
...
PATH_TO_SDM_V2 = os.path.join(BASE_DIR, 'all_aggregate_data/social_distancing/')

PATH_TO_WEEKLY_PATTERNS = os.path.join(BASE_DIR, 'all_aggregate_data/weekly_patterns_data/v1/main-file/')
PATH_TO_V2_WEEKLY_PATTERNS = os.path.join(BASE_DIR, 'all_aggregate_data/weekly_patterns_data/v2/bulk')
...
PATH_TO_CENSUS_BLOCK_GROUP_DATA = os.path.join(BASE_DIR, 'external_datasets_for_aggregate_analysis/census_block_group_data/') # census block group boundaries.

#PATH_FOR_CBG_MAPPER = os.path.join(BASE_DIR, 'old_dfs_scratch0_directory_contents/new_census_data/')
PATH_FOR_CBG_MAPPER = os.path.join(BASE_DIR, 'external_datasets_for_aggregate_analysis/census_block_group_data/')
...
# maps counties to MSAs, consistent with the Experienced Segregation paper. 
# Data was downloaded from https://www2.census.gov/programs-surveys/metro-micro/geographies/reference-files/2017/delineation-files/list1.xls.
PATH_TO_COUNTY_TO_MSA_MAPPING = os.path.join(BASE_DIR, 'external_datasets_for_aggregate_analysis/census_block_group_data/august_2017_county_to_metropolitan_mapping.csv') 
...
PATH_TO_ITDPC_DATA = os.path.join(
    BASE_DIR,
    'external_datasets_for_aggregate_analysis',
    'nytimes_coronavirus_data',
    'covid-19-data','dpc-covid19-ita-province.csv')
...
#MAX_MODELS_TO_TAKE_PER_MSA = 30  # NEW GB 20/05/2023 for rebuttal
...
# ALL_WEEKLY_STRINGS = sorted(
#     [a.replace('-weekly-patterns.csv.gz', '') 
#     for a in os.listdir('/dfs/scratch1/safegraph_homes/all_aggregate_data/weekly_patterns_data/v1/main-file/')])

# ALL_WEEKLY_STRINGS = sorted(
#     [a.replace('-weekly-patterns.csv.gz', '').split('-part')[0] 
#     for a in os.listdir(PATH_TO_V2_WEEKLY_PATTERNS)])

# ALL_WEEKLY_STRINGS = [
#     '2020-03-01', '2020-03-08', '2020-03-15',
...
# V1
# TRAIN_TEST_PARTITION = datetime.datetime(2020, 3, 22)
# MIN_DATETIME = datetime.datetime(2020, 3, 1, 0)
# MAX_DATETIME = datetime.datetime(2020, 3, 29, 23)
...
HIGHLIGHT_MSA = 'Lombardia'
MSAS = ['Lombardia', 'Campania', 'Lazio', 'Veneto',
        'Treviso','Padova','Roma','Firenze','Torino', 'Milano', 'Napoli', 'Bergamo','Varese']

LOWER_PERCENTILE = 5# 2.5
UPPER_PERCENTILE = 95 #97.5
...
MSAS_TO_PRETTY_NAMES = {m:m for m in MSAS}
...
######### OUR CHANGES ######
wave = "second"
data_type = "filter5_rs"

# NEW V3 10/05:
# add a new parameter for different detection lags between
# wave 1 and 2
...
if wave == "first":
    ...
    path = "first"
    DETECTION_LAG = 17
    INITIALIZE_INFECTED_WITH_OBS = False        
elif wave == "second":
    ...
    #MIN_DATETIME = datetime.datetime(2020, 9, 24, 0)
    ...
    path = "second"
    DETECTION_LAG = 0
    INITIALIZE_INFECTED_WITH_OBS = True 
...
CATEGORY_STEPS = {'470': 0.03375, '560': 0.18799, '550': 0.19384, '860': 0.00973}
