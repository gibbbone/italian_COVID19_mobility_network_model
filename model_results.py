...
from model_evaluation import compare_model_vs_real_num_cases, load_model_and_data_from_timestring, compute_loss, plot_slir_over_time
from scipy.stats import scoreatpercentile
from run_one_model import get_lir_checkpoints_and_prop_visits_lost,get_ipf_filename
import helper_methods_for_aggregate_data_analysis as helper

###################################################################################
# Figure 1: model fit
###################################################################################
def get_best_models_for_all_msas(
    df, key_to_sort_by, only_single_best=False, 
    loss_tolerance=ACCEPTABLE_LOSS_TOLERANCE, verbose=False, msa_subset=None):
    ...
    # NEW
    if msa_subset is None:
        msa_to_get = MSAS
    else:
        msa_to_get = msa_subset
    print(msa_to_get)

    for msa in msa_to_get:
...
def get_summary_of_best_models(df, msas, threshold, max_models, loss_key='loss_dict_daily_cases_RMSE', keys=None):
    if keys is None:
...
def plot_model_fit_from_model_and_kwargs(ax,
    ...
    ret_val,_ = compare_model_vs_real_num_cases(nyt_outcomes, min_datetime,
                                    model=model,
...
def plot_best_models_fit_for_msa(
...
def make_param_plausibility_plot(r0_df, make_r0_base_plot=True, make_r0_poi_plot=True, 
    ...
    # NEW_V2
    MSAS = BIGGEST_MSAS
    MSAS_TO_PRETTY_NAMES = {k:k for k in MSAS}
    ...
    if make_r0_base_plot:
        ...
        r0_base = (r0_df[['home_beta', 'R0_base']].groupby(['home_beta']).agg(['min', 'mean', 'max', 'size'])).reset_index()
        r0_base.columns = ['home_beta', 'min', 'mean', 'max', 'n_models_fit']
        
        ax.plot(r0_base['home_beta'],r0_base['mean'])
        ...
        if make_rainbow_plot:
            ...
            g = sns.lineplot(
                ax=ax, data=r0_df.reset_index(drop=True), 
                x='home_beta', y='R0_base', hue='MSA', palette='rainbow', legend=False, hue_order=level_order)
            ax.set_xlim([r0_base['home_beta'].min(), r0_base['home_beta'].max()])
            ax.set_xlabel(r"$\beta_{base}$", fontsize=fontsize)
            ax.set_ylabel("$R_{base}$", fontsize=fontsize)
            ax.plot(ax.get_xlim(), [min_r0_base, min_r0_base], linestyle='--', color='black')
    ...
    if make_r0_poi_plot:
        ...
        r0_poi = (r0_df[['poi_psi', 'R0_POI']].groupby(['poi_psi']).agg(['min', 'mean', 'max', 'size'])).reset_index()
        r0_poi.columns = ['poi_psi', 'min', 'mean', 'max', 'n_models_fit']
        #print(r0_poi)

        ax.plot(r0_poi['poi_psi'],r0_poi['mean'])
        ...
        if make_rainbow_plot:
            ...
            g = sns.lineplot(
                ax=ax, data=r0_df.reset_index(drop=True), 
                x='poi_psi', y='R0_POI', hue='MSA', palette='rainbow', hue_order=level_order)
            ax.set_xlim([r0_poi['poi_psi'].min(), r0_poi['poi_psi'].max()])
            ax.set_xlabel("$\psi$", fontsize=fontsize)
            ax.set_ylabel("$R_{POI}$", fontsize=fontsize)
...
def make_schematic(orig_visits, lesser_extent, shifted, colors, ax):
    ...
    ax.plot_date(
        days, daily_ts, linestyle='-', marker='.', 
        color=colors[0], label='actual',
        linewidth=3)
    daily_ts = get_daily_ts(lesser_extent)
    ax.plot_date(
        days, daily_ts, linestyle='-', marker='.', 
        color=colors[1], label='50% of actual',
        linewidth=3, alpha=0.5)
    daily_ts = get_daily_ts(shifted)
    ax.plot_date(
        days, daily_ts, linestyle='-', marker='.', 
        color=colors[2], label='7 days later',
        linewidth=3, alpha=0.5)
...
def plot_lir_over_time_for_multiple_models(timestrings, ax, label, color, return_CI=False):
    ...
    mean = INCIDENCE_POP * (np.percentile(all_lir, 50, axis=0)) #np.mean(all_lir, axis=0)
...
def make_counterfactual_line_plots(
    counterfactual_df, msa, ax, mode,
    cmap_str='viridis', y_lim=None, return_CI=False, shift_values=None, degree_values=None):
    assert mode in {'degree', 'shift-later', 'shift-earlier', 'shift'}
    if mode == 'degree':
        colors = list(cm.get_cmap(cmap_str, 5).colors)
        colors.reverse()
        if degree_values is None:
            values = [-1, .25, .5, np.nan]
        else:
    ...
    for i, val in enumerate(values):
        if np.isnan(val):
            # plot the best-fit models for comparison. counterfactual_baseline_model provides 
            # the timestrings of the best-fit models to actual data.
            timestrings = msa_df.counterfactual_baseline_model.unique()
            label = 'actual' if mode == 'degree' else '0 days (actual)'
            mean, lower_bound, upper_bound = plot_lir_over_time_for_multiple_models(
                timestrings, ax, label, 'black', return_CI=True)
        else:
            ...
            mean, lower_bound, upper_bound = plot_lir_over_time_for_multiple_models(
                timestrings, ax, label, colors[i], return_CI=True)
        means.append(mean)
...
def get_final_LIR_fraction_for_multiple_models(timestrings):
    ...
    for ts in timestrings:
        model, kwargs, _, _, _ = load_model_and_data_from_timestring(
            ts, load_fast_results_only=False,
...
def get_ratios_to_baselines(msa_df, param_name, param_values, baselines, baseline_timestrings):
    ...
    for i, val in enumerate(param_values):
        ...
        # print(baseline_timestrings)
        # print(msa_val_df.counterfactual_baseline_model)
        try:
            assert list(baseline_timestrings) == sorted(list(msa_val_df.counterfactual_baseline_model))
        except Exception:
            print("Problem with list:")
            print(list(baseline_timestrings))
            print(list(msa_val_df.counterfactual_baseline_model))
        
        final_fracs = get_final_LIR_fraction_for_multiple_models(timestrings)        
        ratios = final_fracs / baselines
        #ratios = final_fracs / baselines[:len(list(msa_val_df.counterfactual_baseline_model))*30]

        cols.append('%s=%s' % (param_name.strip('counterfactual_'), val))
        
        mean = np.percentile(ratios, 50) #np.mean(ratios)
...
def get_counterfactual_ratios_at_datetime(counterfactual_df, msa, degree_values=None, shift_values=None):
    """
    ...
    param_name = 'counterfactual_distancing_degree'
    if degree_values is None:
        param_values = [0, .25, .5, -1]
    else:
        param_values = degree_values

    cols, ratios = get_ratios_to_baselines(
        msa_df, param_name, param_values, baselines, baseline_timestrings)
    
    param_name = 'counterfactual_shift_in_days'
    if shift_values is None:
        param_values = [7, 3, -3, -7]
    else:
        param_values = shift_values
    
    cols2, ratios2 = get_ratios_to_baselines(
        msa_df, param_name, param_values, baselines, baseline_timestrings)
    cols.extend(cols2)
...
def get_poi_densities(poi_characteristics, poi_cbg_visits_list):
    ...
    #print(poi_cbg_visits_list[0].shape, len(poi_characteristics['poi_areas']))
...
def make_boxplot_of_poi_reopening_effects(
    intervention_df, msa_names, poi_and_cbg_characteristics, 
    titlestring, cats_to_plot, filename, only_plot_reopening_impact=False, alpha_target=0):
    """
    ...
    if not only_plot_reopening_impact:
        ...
        if filename is not None:
            plt.savefig(filename, bbox_inches='tight')   
        return combined_df, mean_impact
...
def get_frac_infected_over_time_per_category(results_df, poi_and_cbg_characteristics, msa_name, categories_to_plot):
    ...
    pretty_names = np.array([
        SUBCATEGORIES_TO_PRETTY_NAMES[x] if x in SUBCATEGORIES_TO_PRETTY_NAMES else x for x in poi_categories])
    results_per_seed = []
    for ts in msa_df.timestring.values:
        ...
        for s in range(len(num_cases_per_poi)):  # iterate through seeds
            ...
            for cat in categories_to_plot:
                ...
                # there should be at least 10 POIs in this category
                assert np.sum(cat_idx) > 10  
                # sum over pois within cat
                frac_infected_at_cat_per_day = np.sum(num_cases_per_poi[s][cat_idx], axis=0) / pop_size  
