...
class Model:
    def __init__(self,
        ...
        # just bind variables to the Model object
    ...
    def init_exogenous_variables(
        ...
        mean_dwell=75, ## AGGIUNTA FP 23/12
        square_feet_mean=1076.39, ## added GB 20220901
        square_feet_sigma=240,
        area_model='normal', # added GB 20/05/23
        dwell_model='exponential', # added GB 20/05/23
        risk_update_param=14, # added GB 11/01/24
        ): ## added GB 20220901
        ...
        # ----------------------------- POI variables
        # POI_CBG_PROPORTIONS
        # I don't understand why this step is not skipped
        # when we already have mobility matrices
        ...
        if self.POI_CBG_VISITS_LIST is not None:
            ...
            print(self.POI_CBG_VISITS_LIST[0].shape , self.M, self.N)
        ...
        # NEW_V3: these are set inside fit_disease_model_on_real_data
        # assert ('nyt' not in self.cbg_idx_groups_to_track)
        # self.cbg_idx_groups_to_track['nyt'] = np.arange(self.N)
        ...
        self.RISK_UPDATE = risk_update_param

        # checks
        array_params = [self.POI_FACTORS, self.CBG_SIZES, self.POI_TIME_COUNTS]        
        for arr,arr_str in zip(array_params, ['POI factors','CBG sizes', 'POI time counts']):
            if not (arr >= 0).all():
                print(f"Negative values in {arr_str}: {sum(arr<0)}")
            # NEW?
            #assert (arr >= 0).all()
    ...
    def _build_proportions_matrix(self, proportion_dicts):
        ...
        # only check this if we're not just testing.
        if self.M > 1000: 
            # make sure not very many POIs seem to have lots of people who are coming from outside of the CBG set.
            assert (self.POI_CBG_PROPORTIONS.sum(axis=1) < .5).mean() < 0.01 

    def init_endogenous_variables(self, initial_infection_constant=None):        
        # print("--- Start debugging ----")
        # for n in [self.CBG_SIZES, self.P_SICK_AT_T0, self.num_seeds, self.N]:
        #     print("")
        #     print(i)
        
        # Initialize exposed/latent individuals
        self.P0 = np.random.binomial(
            self.CBG_SIZES.astype(int),
            self.P_SICK_AT_T0,
        ...
        # New V3
        if initial_infection_constant is not None:            
            I_zero = initial_infection_constant
            perc_thresh = np.asarray(1/I_zero)
            pop_percentages = self.CBG_SIZES/sum(self.CBG_SIZES)
            first_redistr = np.ceil(np.where(
                pop_percentages >= perc_thresh, pop_percentages, 0)*I_zero)
            
            leftover = I_zero - sum(first_redistr)
            second_redistr = np.ceil(first_redistr+(first_redistr/I_zero)*leftover)
            remainder = int(I_zero-sum(second_redistr))

            if remainder > 0:        
                positions = np.random.choice(
                    np.arange(len(self.CBG_SIZES)), 
                    abs(remainder), 
                    replace=False)
                assert len(positions) == abs(remainder)
                remainder_array = np.zeros(len(self.CBG_SIZES))
                remainder_array[positions] = 1
                third_redistr = second_redistr + remainder_array
                assert sum(third_redistr) == I_zero
            else:
                positions = np.random.choice(
                    np.nonzero(second_redistr>0)[0],
                    abs(remainder), 
                    replace=False)
                assert len(positions) == abs(remainder)
                remainder_array = np.zeros(len(self.CBG_SIZES))
            ...
            cbg_infected = np.ones((self.num_seeds, len(self.CBG_SIZES)))*third_redistr
            assert np.all([sum(a) == I_zero for a in cbg_infected])
            self.cbg_infected = cbg_infected
        ...
        for group in self.cbg_idx_groups_to_track:
            ...
            for compartment in ['num_out','num_cbgs_with_no_out']:
                self.history[group][compartment] = np.zeros(self.T)  # non-stochastic
            
            for compartment in ['ipf_iter_col_num_out','ipf_iter_row_num_out']:
        ...
        if self.POI_CBG_VISITS_LIST is None:
    ...
    def simulate_disease_spread(
        self, verbosity=24,
        simulate_cases=False, 
        simulate_deaths=False,
        ...
        if self.just_compute_r0:
            ...
            try:
                ...
                print(f"Problems with R0: {self.estimated_R0['R0_base']} | {self.estimated_R0['R0_POI']} |{ self.estimated_R0['R0']}")
    ...
    def get_new_cases(self, t):
        '''
        Determines the number of new cases per CBG. This depends:
        - on the CBG's home infection rate 
        - on the infection rates of the POIs that members from this CBG visited at time t. 
        
        If the model is stochastic, the number of new cases is drawn randomly; 
        otherwise, the expectation of the random variable is used.
        ...
        if self.PSI > 0:
            ...
            if self.use_home_proportion_beta: #default: false
                day = int(t / 24)
        ...
        # if SOCIAL_UPDATE:
            # high_risk_perception = # big todo_rebuttal
            ## calculate proximity matrix
            ## calculate neighbors infections    
            # calculate infection updates

        high_risk_perception = cbg_densities > 0.005        
        infection_updates = 1 - np.random.beta(a=1,b=self.RISK_UPDATE, size=self.N)            
        social_update_rates = np.where(high_risk_perception, infection_updates, 1)
        cbg_base_infection_rates = cbg_base_infection_rates * social_update_rates

        if not t % 120:
            print(f"Behavioral update: share of high risk municipalities {high_risk_perception.sum().sum()/self.N}%")
            print(f"Behavioral update: average decrease in contagion {np.mean(social_update_rates)*100}%")
        ...
        if self.use_aggregate_mobility:  # don't use network data, just use total number of POI visits in this hour
            ...
            num_cases_from_poi = np.random.poisson(np.abs(cbg_mean_new_cases_from_poi))
        ...
        if self.debug:
            ...
            print(f'using poisson approx: expected new cases = {np.sum(self.cbg_num_cases_per_poi)}')
